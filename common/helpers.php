...
function getDataProviderSummary($dataProvider, $options = [])
{
    ...
    if (($pagination = $dataProvider->getPagination()) !== false) {
        ...
        return Html::tag($tag, Yii::t('yii', 'Показаны записи <b>{begin, number}-{end, number}</b> из <b>{totalCount, number}</b>.', [
                'begin' => $begin,
                'end' => $end,
                'count' => $count,
        ...
        return Html::tag($tag, Yii::t('yii', 'Всего <b>{count, number}</b> {count, plural, one{запись} few{записи} many{записей} other{записи}}.', [
            'begin' => $begin,
            'end' => $end,
            'count' => $count,
            'totalCount' => $totalCount,
